<template>
	<div class="mpvue-calendar" ref="calendar">
		<div class="calendar-tools" v-if="!isMonthRange">
			...
			<div class="calendar-info" @click.stop="changeYear">
				<div class="mc-month">
					...
					<div :class="{'mc-month-inner':oversliding ? '' : 'month-transition'}" :style="{'top': monthPosition + unit}"
					 v-if="isIos && 1!=1">
						<view v-for="(m, i) in monthsLoop" :key="i">{{m}}</view>
					</div>
		...
		<table cellpadding="5">
			<div class="mc-head" :class="{'mc-month-range-mode-head': isMonthRange}">
				<div class="mc-head-box">
					<div v-for="(week, index) in weeks" :key="index" class="mc-week">{{week}}</div>
				</div>
			</div>
			<div class='mc-body' :class="{'mc-range-mode': range, 'week-switch': weekSwitch && !isMonthRange, 'month-range-mode': isMonthRange}"
			 v-for="(days, index) in monthRangeDays" :key='index'>
				...
				<tr v-for="(day,k1) in days" :key="k1" :class="{'gregorianStyle': !lunar}">
					<td v-for="(child,k2) in day" :key="k2" :class="{'selected': child.selected, 'mc-today-element': child.isToday, 'disabled': child.disabled, 'mc-range-select-one': rangeBgHide && child.selected, 'lunarStyle': lunar, 'mc-range-row-first': k2 === 0 && child.selected, 'month-last-date': child.lastDay, 'month-first-date': 1 === child.day, 'mc-range-row-last': k2 === 6 && child.selected, 'mc-last-month': child.lastMonth, 'mc-next-month': child.nextMonth}"
					 class="mc-day" :style="itemStyle">
						<div v-if="single && (child.lastMonth || child.nextMonth)"></div>
						<div v-else @click="select(k1, k2, child, $event, index)">
							<view v-if="showToday.show && child.isToday" class="mc-today calendar-date">{{showToday.text}}
							</view>
							<view class='calendar-date' :class="{'mc-date-red': k2 === (monFirst ? 5 : 0) || k2 === 6}" v-else>{{child.day}}</view>
							<div class="slot-element" v-if="!!child.content">{{child.content}}</div>
							<span class="mc-text remark-text" v-if="child.eventName && !clean">{{child.eventName}}</span>
							<div class="mc-dot" v-if="child.eventName && clean"></div>
							<span class="mc-text" :class="{'isLunarFestival': child.isAlmanac || child.isLunarFestival,'isGregorianFestival': child.isGregorianFestival,'isTerm': child.isTerm}"
							 v-if="lunar && (!child.eventName || clean)">{{child.almanac || child.lunar}}</span>
		...
		<div class="mpvue-calendar-change" :class="{'show': yearsShow}">
			<div class="calendar-years" v-if="!weekSwitch">
				<view v-for="y in years" :key="y" @click.stop="selectYear(y)" :class="{'active': y === year}">{{y}}</view>
			</div>
			<div class='calendar-months' :class="{'calendar-week-switch-months': weekSwitch}">
				<view v-for="(m, i) in months" :key="m" @click.stop="changeMonth(i)" :class="{'active': i === month}">{{m}}</view>
			</div>
		</div>
	</div>
</template>
...
<script>
	...
	export default {
		...
		watch: {
			...
			monthRange() {
				// console.log("this.isRendeRangeMode",this.isRendeRangeMode());
				if (this.isRendeRangeMode()) return;
		...
		mounted() {
			...
			this.init();
			// console.log(this.$common.clearZero("2018-01-01"));
			if (uni.getStorageSync("userTicketDate")) {
				let isFind = false;
				this.monthRangeDays.forEach((item, index) => {
					if(isFind)return;
					item.forEach((item1, index1) => {
						if(isFind)return;
						item1.forEach((item2,index2)=>{
							if (item2.date == this.$common.clearZero(uni.getStorageSync("userTicketDate"))) {
								item2.selected = true;
								isFind = true;
								return;
							}
			...
			// console.log("this.monthRangeDays",this.monthRangeDays);	
		},
		beforeDestroy() {
			if (isBrowser) {
		...
		methods: {
			...
			rendeRange(renderer) {
				...
				function formatDateText(fYear, fMonth) {
					...
					return String(fYear).substring(4 - RegExp.$1.length) + RegExp.$2 + String(fMonth).substring(2 - RegExp.$3.length) +
						RegExp.$4;
				}
			...
			select(k1, k2, data, e, monthIndex) {
				...
				const fineEvent = events.filter((item, index) => {
					return item == data.date;
				})
				if (fineEvent.length > 0) {
					e && e.stopPropagation();
					const weekSwitch = this.weekSwitch;
